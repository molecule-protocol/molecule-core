...
// Root file: v2/contracts/tokens/ERC721m.sol
...
abstract contract ERC721m is ERC721, Ownable {
    ...
    function mint(address to, uint256 tokenId) external {
        if (_moleculeMint != address(0)) {
            require(IMoleculeLogic(_moleculeMint).check(to), "ERC721m: account not allowed to mint");
        }
        _safeMint(to, tokenId);
    }
    ...
    function burn(uint256 tokenId) external {
        ...
        if (_moleculeBurn != address(0)) {
            require(IMoleculeLogic(_moleculeBurn).check(owner), "ERC721m: account not allowed to burn");
        }
        _burn(tokenId);
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual override {
        if (_moleculeTransfer != address(0)) {
            require(IMoleculeLogic(_moleculeTransfer).check(from), "ERC721m: sender not allowed to transfer");
            require(IMoleculeLogic(_moleculeTransfer).check(to), "ERC721m: recipient not allowed to receive");
        }
    ...
    function _approve(address to, uint256 tokenId) internal virtual override {
        if (_moleculeApprove != address(0)) {
            address owner = ownerOf(tokenId);
            require(IMoleculeLogic(_moleculeApprove).check(owner), "ERC721m: owner not allowed to approve");
            require(IMoleculeLogic(_moleculeApprove).check(to), "ERC721m: spender not allowed to receive");
        }
