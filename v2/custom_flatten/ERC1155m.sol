...
// Root file: v2/contracts/tokens/ERC1155m.sol
...
contract ERC1155m is ERC1155, Ownable {
    ...
    function mint(address account, uint256 id, uint256 amount, bytes memory data) external {
        if (_moleculeMint != address(0)) {
            require(IMoleculeLogic(_moleculeMint).check(account), "ERC1155m: account not allowed to mint");
        }
    ...
    function burn(address account, uint256 id, uint256 amount) external {
        if (_moleculeBurn != address(0)) {
            require(IMoleculeLogic(_moleculeBurn).check(account), "ERC1155m: account not allowed to burn");
        }
    ...
    function _beforeTokenTransfer(
        ...
        if (_moleculeTransfer != address(0)) {
            require(IMoleculeLogic(_moleculeTransfer).check(from), "ERC1155m: sender not allowed to transfer");
            require(IMoleculeLogic(_moleculeTransfer).check(to), "ERC1155m: recipient not allowed to receive");
        }
    ...
    function _setApprovalForAll(address owner, address operator, bool approved) internal virtual override {
        if (_moleculeApprove != address(0)) {
            require(IMoleculeLogic(_moleculeApprove).check(operator), "ERC1155m: owner not allowed to approve");
        }
